...
function Install-ScriptExecuter {
    $link = "https://raw.githubusercontent.com/Andreas6920/deploy-project/refs/heads/main/resources/Install-ScriptExecuter.ps1"
    $File = Join-path -Path $Env:TMP -Childpath "Install-ScriptExecuter.ps1"
    (New-Object net.webclient).Downloadfile($link, $File)
    . $File; Install-ScriptExecuter}

function Install-BitdefenderTotalSecurity {
...
function Remove-BitdefenderTotalSecurity {
    ...
    $File = Join-path -Path ([Environment]::GetFolderPath("Desktop")) -Childpath "Bit\Bitdefender Uninstaller.exe"
    Write-Host "`t`t`t - Downloading $File"
    (New-Object net.webclient).Downloadfile($link, $File)
    Write-Host "`t`t`t - Starting $File"
    Start-Process -FilePath $File}

if($InstallScriptExecuter){Install-ScriptExecuter}
if($InstallBitdefenderTotalSecurity){Install-BitdefenderTotalSecurity}
if($RemoveBitdefenderTotalSecurity) {Remove-BitdefenderTotalSecurity}
else{
...
Write-host "`tIs BitDefender installed:`t`t" -NoNewline
if(test-path "C:\Program Files\Bitdefender"){Write-host "Yes" -ForegroundColor Green} else {Write-host "No" -ForegroundColor Red}
